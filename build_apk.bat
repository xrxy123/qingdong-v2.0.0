...
REM ============================================================
REM  轻动 (QingDong) v2.0.0 - 本地 APK 构建脚本（无需 Android Studio / Gradle）
REM  前置条件：
...
REM  产物：当前目录下的 qingdong-v2.0.0.apk
REM ============================================================
...
set "KEYSTORE=%OUT%\qingdong.keystore"
if not exist "%KEYSTORE%" (
  "%JAVA_HOME%\bin\keytool" -genkeypair -v -keystore "%KEYSTORE%" -keyalg RSA -keysize 2048 -validity 10000 -alias qingdong -storepass qingdong123 -keypass qingdong123 -dname "CN=QingDong, OU=Fit, O=QingDong, L=CN, S=CN, C=CN"
)
"%BUILD_TOOLS%\zipalign.exe" -p 4 "%OUT%\app-unaligned.apk" "%OUT%\app-aligned.apk"
call "%BUILD_TOOLS%\apksigner.bat" sign --ks "%KEYSTORE%" --ks-key-alias qingdong --ks-pass pass:qingdong123 --key-pass pass:qingdong123 --out "%ROOT%qingdong-v2.0.0.apk" "%OUT%\app-aligned.apk"
if errorlevel 1 exit /b 1

echo.
echo ==================== 构建完成 ====================
echo APK 产物: %ROOT%qingdong-v2.0.0.apk
echo 安装到手机:  adb install "%ROOT%qingdong-v2.0.0.apk"
endlocal
