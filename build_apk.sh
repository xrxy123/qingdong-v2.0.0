#!/usr/bin/env bash
# 轻动 (QingDong) v2.0.0 - 本地 APK 构建脚本（macOS / Linux，无需 Android Studio / Gradle）
...
KEYSTORE="$OUT/qingdong.keystore"
if [ ! -f "$KEYSTORE" ]; then
...
"$BUILD_TOOLS/apksigner" sign --ks "$KEYSTORE" --ks-key-alias qingdong --ks-pass pass:qingdong123 --key-pass pass:qingdong123 --out "$ROOT/qingdong-v2.0.0.apk" "$OUT/app-aligned.apk"
