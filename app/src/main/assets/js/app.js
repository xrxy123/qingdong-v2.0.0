...
(function () {
  ...
  function bindTimer(e) {
    ...
    reset.onclick = () => { running = false; session._started = false; elapsed = 0; tm.textContent = '0:00'; toggle.textContent = '开始'; if (range) range.disabled = false; };
  ...
  function renderPlans() {
    ...
    $$('.plan-card-wrap', view).forEach(wrap => {
      ...
      card.onclick = () => {
        ...
        openSession(planSteps(p), p.name);
      };
      // 删除按钮
  ...
  function renderStats() {
    ...
    $$('.seg button', view).forEach(b => b.onclick = () => { statTab = b.dataset.tab; renderStats(); });
  ...
  // 统计页：左右滑切换日/周/月/年（循环：右滑 day ↔ 上一个不存在，则到 year；左滑 year → day）
  function bindStatSwipe(view) {
    ...
    const onMove = (e) => {
      if (!tracking) return;
      // 不 preventDefault：浏览器可能会忽略 passive，并影响 seg/canvas 上的交互
    };
    const onEnd = (e) => {
      if (!tracking) return;
      tracking = false;
      ...
      if (dx < 0 && idx < tabs.length - 1) { statTab = tabs[idx + 1]; renderStats(); }   // 左滑：next
      else if (dx > 0 && idx > 0) { statTab = tabs[idx - 1]; renderStats(); }            // 右滑：prev
    };
    if (window.PointerEvent) {
      view.onpointerdown = onStart;
      view.onpointermove = onMove;
      view.onpointerup = onEnd;
      view.onpointercancel = onEnd;
    } else {
      view.ontouchstart = onStart;
      view.ontouchmove = onMove;
      view.ontouchend = onEnd;
    }
  }
  ...
  function renderProfile() {
    ...
    // 版本升级：跳转到夸克网盘最新版本（window.open 失败时回退 location.href，兼容安卓 WebView）
  ...
  function openConfirm(title, msg, onYes) {
    openModal(`
      ...
      <div class="session-actions">
        ...
        <button class="btn coral" id="cfYes">确认清空</button>
      </div>`);
    $('#cfNo').onclick = closeModal;
    $('#cfYes').onclick = () => { closeModal(); if (onYes) onYes(); };
  }
  ...
  function openDataIO() {
    ...
    $('#doExport').onclick = () => {
      ...
      a.download = `轻动数据备份_${ts.getFullYear()}${pad(ts.getMonth() + 1)}${pad(ts.getDate())}_${pad(ts.getHours())}${pad(ts.getMinutes())}.json`;
      a.href = url; document.body.appendChild(a); a.click(); a.remove();
      setTimeout(() => URL.revokeObjectURL(url), 1000);
      toast('数据已导出到本机');
      closeModal();
    };
    $('#doImport').onclick = () => { $('#ioFile').click(); };
    $('#ioFile').onchange = (ev) => {
  ...
  function exportReminderICS(r) {
    ...
    a.download = `轻动提醒_${safeName}_${r.time}.ics`;
